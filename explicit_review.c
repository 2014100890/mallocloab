...
1. LIFO 방식으로 구현하셨네요.
2. 이해하기 쉽게 주석을 잘 달아주신 것 같습니다.
3. 226번째 줄 저도 동일하게 짜긴 했는데 직관적으로 이해하기는 편하지만 계산을 낭비하는 것이 아닌가 생각했습니다.
size_t prev_alloc = GET_ALLOC(FTRP(PREV_BLKP(bp)));
 바로 이전 bp-DSIZE에 위치한 Footer를 보고 Alloc 여부를 판단할 수 있는데, 현재는 bp-DSIZE에 위치한 Footer에서 크기를 찾아서 PREV BLKP의 HDR_BLKP에 들어간 뒤 다시 사이즈를 계산해서 FOOTER에 접근한다고 생각이 돼서요.
4. Block을 Free 시킬 때의 로직에 대해서는 별다른 설명을 안적어주셨네요. 저도 똑같이 짜서 이해하기는 쉬웠습니다.
5. realloc 함수가 기본으로 주어진 realloc 상태인 것 같습니다. 옆에 가용블럭이 있으면 늘려주는 방식도 추가해보면 좋을 것 같습니다.
...
int mm_init(void)
{
    ...
    free_listp = NULL;
    if (extend_heap(CHUNKSIZE/WSIZE) == NULL){ // 칸 몇칸 넣을지 개수 
        return -1;
    }
#endif
    #ifdef NEXT_FIT
